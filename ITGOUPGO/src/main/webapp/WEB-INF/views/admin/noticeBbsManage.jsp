...
<body>
    ...
    <div class="layout">
            ...
            <ul>
                ...
                <li id="postManageMenu" class="menu-open">
                    ...
                    <ul class="submenu">
                        ...
                        <li><a href="<c:url value="/admin/noticeBbsManage" />" class="sub-active">공지사항 게시판</a></li>
        ...
        <main>
            <h1>📝 공지사항 게시판 관리</h1>
            <p>공지사항 게시판에 등록된 게시글 목록을 확인하고 관리할 수 있습니다.</p>
          
            <form id="bulkDeleteForm" action="<c:url value="/admin/deleteNoticeBbsCheck" />" method="post" style="display:none;"></form>
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th style="width:50%;">제목</th> <%-- [변경] 작성자 제거로 인해 너비 증가 --%>
                        <%-- <th style="width:15%;">작성자</th> <-- 이 필드를 제거합니다. --%>
                ...
                <tbody>
                    <c:choose>
                        <c:when test="${empty noticeBbsList}"> 
                            <tr><td colspan="6"><c:if test="${not empty searchKeyword}">"${searchKeyword}"에 대한 검색 결과가 없습니다.</c:if><c:if test="${empty searchKeyword}">등록된 게시글이 없습니다.</c:if></td></tr> <%-- [변경] colspan 7 -> 6 --%>
                        </c:when>
                        <c:otherwise>
                            <c:forEach var="post" items="${noticeBbsList}"> 
                                <tr class="${post.noticeDeleteStatus eq 'Y' ? 'deleted-post' : ''}"> 
                                    <td><input type="checkbox" class="itemCheckboxPost" name="bbsNoList" value="${post.noticeNo}"
                                                <c:if test="${post.noticeDeleteStatus eq 'Y'}">disabled</c:if>></td> 
                                    ...
                                    <td class="text-start">
                                        <c:choose>
                                            <c:when test="${post.noticeDeleteStatus eq 'Y'}"> 
                                                <span class="text-danger">❌ [삭제됨]</span> ${post.noticeTitle} 
                                            </c:when>
                                            <c:otherwise>
                                                <a href="<c:url value="/admin/noticeBbsPostDetail?bbsNo=${post.noticeNo}" />"> 
                                                    ${post.noticeTitle} 
                                                </a>
                                            </c:otherwise>
                                        </c:choose>
                                    ...
                                    <td>
                                        <fmt:formatDate value="${post.noticeCreateDate}" pattern="yyyy-MM-dd" /> 
                                    </td>
                                    
                                    <td>${post.noticeView}</td> 
                                    
                                    <td class="action-cell">
                                        <c:choose>
                                            <c:when test="${post.noticeDeleteStatus eq 'N'}"> 
                                                <button class="btn btn-danger"
                                                    onclick="if(confirm('${post.noticeNo}번 게시글을 정말 삭제하시겠습니까?')) { deleteSinglePost(${post.noticeNo}); }"> 
                                                    삭제
                                                </button>
                                            </c:when>
                                            <c:otherwise>
            ...
            <div class="pagination-container">
                <c:choose>
                    ...
                    <c:otherwise>
                        <c:set var="pageLinkUrl" value="/admin/noticeBbsManage" /><c:set var="searchQueryParam" value="" /> 
                    </c:otherwise>
                </c:choose>
...
<script>
    ...
    /*
    // [제거] 원본 템플릿에 searchForm 태그가 없으므로 이 스크립트 블록은 제거합니다.
    ...
    */
    ...
    function deleteSinglePost(noticeNo) {
        if(confirm(noticeNo + '번 게시글을 정말 삭제하시겠습니까?')) { 
            location.href='<c:url value="/admin/deleteNoticeBbs" />?bbsNo=' + noticeNo; 
        }
    }
