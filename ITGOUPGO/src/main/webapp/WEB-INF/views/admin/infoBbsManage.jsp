    ...
    <div class="layout">
        ...
        <aside class="sidebar">
            <ul>
                ...
                <%-- '정보 공유 게시판'이 현재 페이지이므로 'menu-open' 및 'active' 설정 --%>
                <li id="postManageMenu" class="menu-open">
                    ...
                    <ul class="submenu">
                        ...
                        <li><a href="<c:url value="/admin/infoBbsManage" />" class="sub-active">정보 공유 게시판</a></li>
        ...
        <main>
            <h1>📝 정보 공유 게시판 관리</h1>
            <p>정보 공유 게시판에 등록된 게시글 목록을 확인하고 관리할 수 있습니다.</p>


            <form id="bulkDeleteForm" action="<c:url value="/admin/deleteInfoBbsCheck" />" method="post" style="display:none;"></form>
            ...
            <table>
                ...
                <tbody>
                    <c:choose>
                        ...
                        <c:otherwise>
                            <c:forEach var="post" items="${infoBbsList}">
                                <tr class="${post.infoDeleteStatus eq 'Y' ? 'deleted-post' : ''}">
                                    
                                    <%-- [수정] bbsNo -> infoNo --%>
                                    <td><input type="checkbox" class="itemCheckboxPost" name="bbsNoList" value="${post.infoNo}"></td>
                                    <td>${post.infoNo}</td>
                                    
                                    <td class="text-start">
                                        <c:choose>
                                            <c:when test="${post.infoDeleteStatus eq 'Y'}">
                                                <%-- [수정] bbsTitle -> infoTitle --%>
                                                <span class="text-danger">❌ [삭제됨]</span> ${post.infoTitle}
                                            </c:when>
                                            <c:otherwise>
                                                <%-- [추가] 삭제되지 않은 경우 상세 페이지 링크 및 [수정] bbsTitle -> infoTitle, bbsNo -> infoNo --%>
                                                <a href="<c:url value="/admin/infoBbsDetail?infoNo=${post.infoNo}" />">${post.infoTitle}</a>
                                            </c:otherwise>
                                        </c:choose>
                                    </td>
                                    
                                    <%-- [수정] memberId -> memberNick (VO 필드명 기준) --%>
                                    <td>${post.memberNick}</td> 
                                    
                                    <%-- [수정] createDate -> infoCreateDate --%>
                                    <td><fmt:formatDate value="${post.infoCreateDate}" pattern="yyyy-MM-dd" /></td>
                                    
                                    <%-- [수정] bbsCount -> infoView --%>
                                    <td>${post.infoView}</td>
                                        ...
                                        <c:choose>
                                            ...
                                            <c:otherwise>
                                                <button class="btn btn-danger"
                                                    <%-- [수정] bbsNo -> infoNo --%>
                                                    onclick="if(confirm('${post.infoNo}번 게시글을 정말 삭제하시겠습니까?')) { location.href='<c:url value="/admin/deleteInfoBbs?infoNo=${post.infoNo}" />'; }">
                                                    삭제
                                                </button>
                                            </c:otherwise>
                                        </c:choose>
            ...
            <div class="pagination-container">
                <c:choose>
                    ...
                    <c:otherwise>
                        <c:set var="pageLinkUrl" value="/admin/infoBbsManage" /><c:set var="searchQueryParam" value="" />
                    </c:otherwise>
                </c:choose>
...
<script>
    ...
    // NOTE: searchForm은 JSP에서 보이지 않아 주석 처리합니다.
    /*
    ...
    */
    ...
    function confirmBulkDelete() {
        // 💡 [수정] 삭제 상태가 'N'인 게시글(deleted-post 클래스가 없는 행)만 일괄 삭제 대상으로 선택
        ...
        if (confirm('총 ' + selectedCheckboxes.length + '건의 게시글을 일괄 삭제 처리하시겠습니까?')) {
            ...
            selectedCheckboxes.forEach(checkbox => {
                ...
                hiddenInput.type = 'hidden'; hiddenInput.name = 'bbsNoList'; hiddenInput.value = checkbox.value;
